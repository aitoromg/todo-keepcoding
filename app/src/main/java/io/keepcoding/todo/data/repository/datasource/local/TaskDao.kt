...
abstract class TaskDao {
    ...
    @Query("SELECT * FROM tasks WHERE parent_id IS NULL ORDER BY priority_level DESC, created_at DESC")
    abstract fun getAll(): Single<List<TaskEntity>>

    @Query("SELECT * FROM tasks WHERE parent_id IS NULL ORDER BY priority_level DESC, created_at DESC")
    abstract fun observeAll(): Flowable<List<TaskEntity>>

    @Query("SELECT * FROM tasks WHERE parent_id = :parentId ORDER BY priority_level DESC, created_at DESC")
